...
- **Diep (per groep, build-time):** een `INSTALL_*`-toggle op `false` in `.env` plus image-rebuild en volume-recreate (zie [Optionele componenten](../claude-sandbox/docs/opstarten-en-afsluiten.md#optionele-componenten)). Pas nodig als je écht wil zien hoe de container draait zonder dat de plugin ooit geïnstalleerd is — bijvoorbeeld om autoload-effecten of disk-footprint te onderzoeken.
