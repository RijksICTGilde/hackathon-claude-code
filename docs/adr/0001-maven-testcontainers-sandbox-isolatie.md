...
- **De optionele sshd** (§2.3.4) voegt inbound oppervlak toe. De poort wordt
  ...
  luistert sshd op alle adressen, maar `init-firewall.sh` dropt inbound naar de
  SSH-poort van elke bron behalve de gateway. Dat is een drempel, geen
  authenticatie: bron-IP op een gedeelde bridge is te spoofen. De daemon
