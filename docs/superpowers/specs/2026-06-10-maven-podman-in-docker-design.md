...
| Kan `NET_ADMIN`/`NET_RAW` uit de bounding set bij de drop naar `claude`? | De firewall is dan al opgezet. `compose.override.podman-linux.yml` stelt echter dat pasta `NET_ADMIN` nodig heeft voor de tap op `/dev/net/tun`. Met de setuid-strip is de bounding set al veel minder bereikbaar (geen setuid-route naar caps), dus dit is aanvullende defense-in-depth | nog te meten |
| Kan de proc/sysfs-mount-bypass in het AppArmor-profiel zelf geblokkeerd worden (`deny mount fstype=proc,` + bind-mount-denies) zonder geneste podman te breken? | De `/proc/sys`-deny is padgebonden; een verse proc-mount omzeilt hem. De setuid-strip sluit de weg naar de daarvoor benodigde `CAP_SYS_ADMIN`, maar een mount-deny zou het als extra laag sluiten. crun's proc-mount in de nested userns erft vermoedelijk ons profiel, dus dit vergt waarschijnlijk een child-profiel voor de nested runtime | nog te meten |
