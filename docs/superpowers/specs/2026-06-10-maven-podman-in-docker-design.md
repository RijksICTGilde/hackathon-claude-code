...
| `claude-sandbox/compose.override.podman.yml` | `devices: [/dev/net/tun, ${PODMAN_FUSE_DEVICE:-/dev/null}]`, `security_opt: [seccomp=<profiel>, apparmor=<profiel>, systempaths=unconfined, label=disable]`, `PODMAN_STORAGE_DRIVER`-env |
