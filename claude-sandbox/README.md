...
- **Host-key blijft van root**: `640 root:claude` in een `750 root:claude`-directory — sshd draait als `claude` en moet de sleutel kunnen lezen, maar zonder schrijfrecht op de directory kan hij hem niet vervangen of unlinken. Dat belet rotatie van de identiteit, niet het gebruik ervan: lezen is genoeg om zelf een sshd met dezelfde sleutel op te zetten (zie 'wat het níet dekt').
