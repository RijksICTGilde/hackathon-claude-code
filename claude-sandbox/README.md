...
- **De regels dragen geen tijdstempel.** OpenSSH laat de tijd aan syslog over en schrijft achter `-E` alleen de kale boodschap. Er draait hier geen logdaemon die hem toevoegt, dus je kunt wel zien wie er binnenkwam en met welke sleutel, en in welke volgorde, maar niet wanneer. De container-start staat wél getimestampt in `docker logs`, dus regels zijn te plaatsen tussen twee starts. Een logdaemon toevoegen brengt een eigen rechtenmodel mee — een syslog-socket is per ontwerp voor elk lokaal proces schrijfbaar — en dat is een aparte afweging.
