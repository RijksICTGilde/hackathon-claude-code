...
- **Auth-logging**: sshd logt via syslog naar `/var/log/sshd/sshd.log` op een eigen volume. Het bestand is `640 root:claude` en `/var/log` is van root: root schrijft, `claude` leest mee maar kan bestaande regels niet aanpassen, verwijderen of het pad omleggen. `LogLevel VERBOSE` zet de key-fingerprint erbij. De daemon is `busybox syslogd`, die de tijd voor elke regel zet en op grootte roteert (1 MB, drie bestanden). Zonder syslogd zou elke login spoorloos verdwijnen, en met `-E` naar een bestand zouden de regels geen datum of tijd dragen.
...
- **Valt syslogd weg, dan verdwijnt het spoor stil.** `syslog()` schrijft naar een socket en is best-effort: sshd merkt niet dat er niemand meeleest en blijft gewoon logins accepteren. SSH-toegang valt er niet door stil — dat is de reden dat het zo werkt — maar er is ook geen alarm. De entrypoint controleert bij de start dat `/dev/log` bestaat en houdt sshd anders uit; daarna is er geen bewaking meer.
- **De tijdstempel draagt geen jaartal.** syslog schrijft `Mon DD HH:MM:SS`, zoals gebruikelijk in dat formaat. Voor een spoor dat een volume-recreate overleeft is dat een beperking: bij regels ouder dan een jaar is de datum niet meer eenduidig.
