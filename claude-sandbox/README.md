...
- **Host-key blijft van root**: `640 root:claude` in een `750 root:claude`-directory — sshd draait als `claude` en moet de sleutel kunnen lezen, maar zonder schrijfrecht op de directory kan hij hem niet vervangen of unlinken.
...
- **De auth-log verdwijnt bij een recreate.** Een containerlog bestaat per container. Wil je het spoor bewaren, stuur de containerlog dan naar een bestemming buiten de sandbox.
...
- **Het auth-spoor staat onder dezelfde controle.** sshd draait als `claude` en schrijft zijn log dus als `claude`. Wil je een auth-log die de gelogde partij niet kan aanpassen, dan moet die de container uit.
- **Geen privilege separation.** sshd draait als `claude` en kan daardoor zijn pre-auth-proces niet naar een aparte, rechtenloze gebruiker afsplitsen. De winst is dat een lek geen root oplevert; de prijs is dat OpenSSH's eigen scheiding tussen pre- en post-auth wegvalt.
