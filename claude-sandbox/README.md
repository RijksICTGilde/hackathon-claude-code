...
- **Host-key blijft van root**: `640 root:claude`, zodat sshd hem als `claude` kan lezen maar de ingesloten partij hem niet kan vervangen — anders kiest die zelf welke identiteit Kepler in `known_hosts` pint.
...
- **sshd heeft geen capabilities**: hij draait als `claude` na de privilege-drop, dus de `NET_ADMIN`/`NET_RAW` die de container voor de firewall heeft zijn voor hem niet bereikbaar.
