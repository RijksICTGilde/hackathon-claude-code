...
- **`ANTHROPIC_API_KEY` staat buiten sshd zelf**: de daemon wordt gestart met `env -u ANTHROPIC_API_KEY`, zodat een pre-auth-lek in OpenSSH de sleutel niet uit zijn eigen omgeving kan lezen. Dat beschermt de daemon, niet de sessie: elk proces van `claude` draagt de variabele en een sessie is dezelfde uid. Gebruik bij voorkeur `claude login`.
