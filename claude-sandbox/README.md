...
- **De host-identiteit is niet geheim voor de ingesloten partij.** `claude` kan de host-key lezen. Dat is genoeg om sshd te doden (zelfde uid), zelf op dezelfde poort te binden met diezelfde sleutel, en Kepler ziet geen `known_hosts`-mismatch. Daarmee staat ook de rest van `kepler.conf` — `PermitOpen`, `AllowTcpForwarding local`, `AllowAgentForwarding no` — onder controle van de partij die ze zouden moeten beperken. Dat is inherent aan een sshd die als `claude` draait: een niet-root sshd kan alleen sessies leveren als zichzelf.
