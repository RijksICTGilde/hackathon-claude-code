...
- **Auth-logging**: sshd logt met `-e` naar de containerlog, zichtbaar met `docker compose logs` (er draait geen syslog-daemon; zonder dit verdwijnt elke login spoorloos). `LogLevel VERBOSE` zet de key-fingerprint erbij, en de rotatie van je logdriver geldt.
...
- **De auth-log leeft zolang de containerlog leeft.** Auth-events gaan naar `docker logs`; hoelang ze bewaard blijven bepaalt je logdriver. Wil je ze langer houden, stuur de containerlog dan naar een externe bestemming.
