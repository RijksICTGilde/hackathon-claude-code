...
- **De loopback-binding geldt alleen vanaf de host.** Binnen de container luistert sshd op `0.0.0.0:22`. `init-firewall.sh` dropt inbound naar poort 22 van elke bron behalve de gateway, zodat een andere container op hetzelfde compose-netwerk er niet rechtstreeks bij kan. Die regel staat er alleen bij `ENABLE_SSHD=true`; de rest van het bridge-subnet blijft voor het overige verkeer toegestaan.
