...
`rtk` valt onder allebei: het install-script wordt per release-tag gevendord (`rtk` v0.49.0) en de binary is via `RTK_VERSION` in de `Dockerfile` gepind. Een losse SHA-pin is daar niet nodig, omdat het script de binary zelf verifieert tegen de `checksums.txt` van dezelfde release.
