...
- **`openssh-server` komt ongepind uit apt** en valt buiten Dependabot. CI scant de sshd-variant wel met Trivy, dus een kwetsbare sshd blokkeert de PR; de fix is een rebuild. Bewust geaccepteerde bevindingen staan met reden in `.trivyignore`. Met een luisterende dienst erbij is regelmatig herbouwen geen hygiëne meer maar een beveiligingseis.
