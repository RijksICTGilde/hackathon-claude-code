...
- **Auth-logging**: sshd logt naar `/home/claude/.sshd-log/sshd.log` op het `claude-home` volume (er draait geen syslog-daemon; zonder dit verdwijnt elke login spoorloos). Het bestand is `640 root:claude`: root schrijft, `claude` leest mee maar kan bestaande regels niet aanpassen of verwijderen. De directory eromheen staat wel in `/home/claude`, dus wegschuiven kan hij hem — de entrypoint maakt dan bij de volgende start een verse aan. `LogLevel VERBOSE` zet de key-fingerprint erbij.
...
- **De auth-log roteert niet en verlaat de container niet.** Het bestand staat op het volume en overleeft een recreate, maar groeit onbegrensd en verdwijnt bij een volume-recreate. Wil je het spoor als bewijsmateriaal, stuur het dan naar een bestemming buiten de sandbox — een log die de gelogde partij kan lezen blijft anders het enige exemplaar.
