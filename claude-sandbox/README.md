...
- **De auth-log roteert niet en verlaat de container niet.** Het bestand staat op het `sshd-log` volume en overleeft een container-recreate, maar groeit onbegrensd en verdwijnt zodra je dat volume verwijdert. Het staat los van `claude-home`, dus het spoor blijft staan als je dat volume weggooit om een environment-variabele te wijzigen. Wil je het als bewijsmateriaal, stuur het dan naar een bestemming buiten de sandbox — binnen de container blijft het het enige exemplaar, en wie root in de container heeft kan het alsnog herschrijven.
...
> **Het auth-spoor staat op een eigen volume.** `sshd-log` wordt door compose aangemaakt en staat los van `claude-home`: verwijder je dat laatste om een environment-variabele te wijzigen, dan blijven de auth-events staan. Wil je het spoor wél kwijt, verwijder dan `docker volume rm claude-sandbox_sshd-log` apart.
