...
- **`ANTHROPIC_API_KEY` blijft leesbaar binnen de sessie.** De entrypoint draait als PID 1 met die variabele in zijn omgeving en na de privilege-drop als `claude`; een SSH-sessie is dezelfde uid en leest `/proc/1/environ`. De env-scrub op de daemon verkleint het pre-auth-oppervlak, ze maakt de sleutel niet onbereikbaar voor wie binnen is.
