...
- **Host-key blijft van root**: `640 root:claude` in een `700 root:root`-directory, zodat `claude` hem niet kan vervangen of unlinken.
...
- **De loopback-binding geldt alleen vanaf de host.** Binnen de container luistert sshd op `0.0.0.0:2222`, en `init-firewall.sh` accepteert inbound vanaf het hele bridge-subnet. Een andere container op datzelfde compose-netwerk bereikt die poort dus rechtstreeks, langs de `127.0.0.1`-publish om. Draai geen onvertrouwde containers op dit netwerk. (#101 beperkt dat tot de gateway; die PR zet `SSHD_PORT` mee om als hij hierop rebaset.)
